...
class DisplayController(
    ...
    fun getDisplayPage(@PathVariable id: UUID): ResponseEntity<String> {
        val auction = auctionQueryUseCases.getAuction(id).getOrNull() ?: throw BusinessLogicException("Auction not found", "error.auction_not_found")
    ...
    fun getPublicAuctionState(@PathVariable id: UUID): ApiResponse<AuctionStateSnapshot> {
        val snapshot = auctionComplexQueryUseCases.getStateSnapshot(id).getOrNull() ?: throw BusinessLogicException("Auction not found", "error.auction_not_found")
    ...
    private fun buildPublicStream(
        ...
        val auction = auctionQueryUseCases.getAuction(auctionId).getOrNull() ?: throw BusinessLogicException("Auction not found", "error.auction_not_found")
        ...
        val snapshotResult = auctionComplexQueryUseCases.getStateSnapshot(auctionId)
        if (snapshotResult is com.crichere.common.domain.Result.Success) {
            ...
            val logsResult = auctionQueryUseCases.getAuditLogsSince(auctionId, replayFrom)
    ...
    fun getPublicAuctionView(@PathVariable token: String): ApiResponse<AuctionStateSnapshot> {
        val auction = auctionQueryUseCases.getAuctionByToken(token).getOrNull() ?: throw BusinessLogicException("Auction not found", "error.auction_not_found")
        if (auction.status != AuctionStatus.LIVE) {
            throw BusinessLogicException("Auction is not LIVE", "error.auction_not_live")
        }
        val snapshot = auctionComplexQueryUseCases.getStateSnapshot(auction.id).getOrNull() ?: throw BusinessLogicException("Auction not found", "error.auction_not_found")
        return ResponseHelper.success(data = snapshot)
    }
